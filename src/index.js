...
import {
  getUserLivingLocation,
  generateCurrentLocation,
} from "./getLivingCityTime.js";
import {
  displayForecastCard,
  chooseHourlyCardDate,
  displayHourlyData,
  displaySnowForecast,
} from "./createCard.js";
import {
  getLocationName,
  generateSearchedLocation,
  extractCityNameFromTimezone,
} from "./getSearchedCity.js";
...
export async function fetchWeatherAPI(
  ...
  try {
    ...
    const response = await fetch(
      `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${locationName}/today/next${daysNumber - 1}days?unitGroup=${unitGroup}&key=T6AXBLSPSAQCWVWEGELLNB657`,
      { mode: "cors" },
    );
    ...
    if (isSearch) {
      ...
      upperContainer.style.gridTemplateColumns =
    ...
    if (cityNameElement && locationData.timezone) {
      const extractedCityName = extractCityNameFromTimezone(
...
function displayAlertMessage(data) {
  ...
  alertMessage.innerHTML = data.alerts
